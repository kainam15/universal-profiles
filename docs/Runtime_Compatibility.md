...
下方可滚动详情优先显示继承路径、本层完整依赖及来源，再显示大小拆分、删除释放估算、关系证据、后代数量、完整 ID、标签、模型、创建时间和容器引用。
...
依赖只在选中镜像后的详情区显示，可滚动查看全部包；Torch、Transformers、Diffusers 等关键包及其精确版本排在前面。
这里的“本层”指树中的逻辑构建阶段，
...
无需增加依赖、Docker 查询或定时刷新。光标切换仅重绘可见树区域，不进入正式测量窗口。
