...
搜索支持模型 ID、逻辑环境名、标签、环境摘要及镜像 ID。三个视图共用筛选与勾选：

| 视图 | 用途与交互 |
| --- | --- |
| 镜像树（默认） | 显示层级、完整/新增大小和容器引用数；`←/→` 折叠/展开，空格或点击节点勾选。搜索保留淡色祖先节点作为上下文。 |
...
下方可滚动详情优先显示继承路径、大小拆分与删除释放估算，再列出关系证据、后代数量、完整 ID、标签、模型、创建时间和容器引用。
...
树和列表复用其原生 Tree/DataTable 及 Pilot 交互测试。层视图参考 [Dive](https://github.com/wagoodman/dive)
