...
* **Compute Throughput**: `model_mflop_per_request`, `compute_mflops_app`, and packet-latency-adjusted `compute_mflops` from the default Intel Advisor / `ncu` vendor path, or from the PyTorch profiler path when `--compute-profile-tool auto` or `torch` is selected.
...
* **compute_profile_plan.json**: Per-scale FLOP profile data used to fill compute columns in `result_all.csv`. The default path uses Advisor/ncu; explicit PyTorch profiler mode is available with `--compute-profile-tool auto` or `torch`. Profiler failures record diagnostics here and keep compute values as `nan`.
