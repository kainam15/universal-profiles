...
ARG PYTHON_BASE_IMAGE=docker.m.daocloud.io/library/python:3.10-slim@sha256:fd76ade0c607f27677bc04be3c60749f400eedc941d9e72967e19a4cedff80c2
FROM ${PYTHON_BASE_IMAGE} AS framework
ARG TORCH_INDEX_URL=https://download.pytorch.org/whl/cu128
ARG COMMON_REQUIREMENTS_LOCK=dockerfiles/locks/common-cu128.txt
RUN apt-get update \
...
COPY ${COMMON_REQUIREMENTS_LOCK} /opt/acprof/common-requirements.lock
RUN --mount=type=cache,target=/root/.cache/pip \
    pip install --no-deps -r /opt/acprof/common-requirements.lock \
