...
符合受限接口的自定义多模态 pipeline 可[自动生成模型契约](docs/Runtime_Compatibility.md#自动生成模型契约m1m3)；其余接口或未解决依赖可使用[本地模型声明](docs/Runtime_Compatibility.md#本地模型声明与自定义-pipeline)，通过独立推理验证后再采集。
